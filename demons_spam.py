...
def join_demon(rectangles, demon_to_repeat, demon_to_check_against, difficulty, search_for_how_many_frames = 30):
    loop_time = time()
    while True:
        click_until_found(rectangles, demon_to_repeat, difficulty, 0.85, 0.85)
        while True:
            if clicked := True:
                ...
                if rectangles.size != 0:
                    ...
                    for i in range(0, 1750 , 1):
                        (screenshot, window_location) = wincap.get_screenshot()
                        rectangles = demon_join.find(screenshot, 0.975)
                        ...
                        if rectangles.size != 0:
                            ...
                            if frames >= 15:
                                ...
                                (screenshot, window_location) = wincap.get_screenshot()
                                rectangles = demon_to_repeat.find(screenshot, 0.85)
                                output_im = demon_to_repeat.draw_rectangles(screenshot, rectangles)
                                cv.imshow('Matches', output_im)
                                if rectangles.size != 0:
                                    click_until_found(rectangles, demon_to_repeat, difficulty, 0.75, 0.7)
...
while(True):
        ...
        #pick a demon and join raid
            if True:
                ...
                while True:
                    ...
                    join_demon(rectangles, fattie, skinny, hell_diff, 30)
                    #join the raid
                    while True:
    ...
    if True:
        while True:
            (screenshot, window_location) = wincap.get_screenshot()
            rectangles = demon_ok.find(screenshot, 0.75)
            ...
            if rectangles.size != 0:
                click_until_found(rectangles, demon_ok, tavern, 0.7, 0.8)
                print("pressed ok")
                break
    if cv.waitKey(1) == ord('q'):
        cv.destroyAllWindows()
